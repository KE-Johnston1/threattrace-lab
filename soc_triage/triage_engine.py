...
def triage_alert(case: TriageInput) -> TriageResult:
    """Return the least-assumptive assessment supported by supplied evidence."""
    gaps = []
    ...
    if not case.owner_verified or not case.authorization_verified:
        return TriageResult(
            "Insufficient Evidence", "Low", "Continue investigation",
            "The alert identifies suspicious authentication activity, but ownership and authorization are not established.",
            tuple(gaps),
        )

    if case.successful_login and not case.post_auth_reviewed:
        return TriageResult(
            "Requires Investigation", "Medium",
            "Review the successful session and correlate endpoint/network evidence",
            "A successful authentication followed the detected failure pattern; validation is required before any compromise conclusion.",
            tuple(gaps),
        )

    if gaps:
        return TriageResult(
            "Requires Investigation", "Medium", "Collect the remaining contextual evidence",
            "Some contextual checks remain incomplete, so the alert should not be closed as expected activity yet.",
            tuple(gaps),
        )

    return TriageResult(
        "Expected", "High", "Document rationale and close if the applicable playbook permits",
        "Ownership, authorization, timing, network context, change/testing context, and post-authentication review are verified.",
        (),
    )
